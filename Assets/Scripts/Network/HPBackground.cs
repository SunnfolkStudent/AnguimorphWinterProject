...
public class HPBackground : NetworkBehaviour
{
    ...
    void Start()
    {
	    ...
	    if (isLocalPlayer)
	    {
		    gameObject.transform.parent.transform.position = CardGameManager.singleton.hostSpawn.position;
	    }
	    else
	    {
		    gameObject.transform.parent.transform.position = CardGameManager.singleton.clientSpawn.position;
	    }
    }
